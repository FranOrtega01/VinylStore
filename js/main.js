...
function removeOneItemCart(idProducto){//Remover una cantidad del carrito
    ...
    indexCart !== -1 && carrito[indexCart].cantidad == 1 ? carrito.splice([indexCart],1) : carrito[indexCart].cantidad--;
    refreshCart();
...
function generateFilters(){
    ...
    albumList.forEach(album => {
        ...
        if(!a) genders.push(album.gender)
    })
...
let acumuladorCoin = ``;
coins.forEach(el => {
    el === coinState ? acumuladorCoin += `<option value='${el}' selected>${el}</option>` : acumuladorCoin += `<option value='${el}'>${el}</option>`;
});
...
radioInputs.forEach(input => input.addEventListener('click', uncheckRadioInput))
